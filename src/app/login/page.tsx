          ...
          <div>
            ...
            <div className="mb-6 space-y-1">
              ...
              <p className="text-xs text-slate-500 font-medium">
                Access your store ERP management system.
              </p>
            </div>

            {/* Authentication Mode Switcher: PIN vs Password */}
            <div className="flex bg-slate-100 p-1 rounded-[5px] border border-[#cbcbcb] mb-5 text-xs font-bold">
              <button
                type="button"
                onClick={() => {
                  setLoginMode('pin');
                ...
                className={`flex-1 py-2 text-center rounded-[4px] transition-all flex items-center justify-center gap-1.5 ${
                  loginMode === 'pin'
                    ? 'bg-white text-emerald-800 shadow-sm border border-[#cbcbcb]'
                    : 'text-slate-600 hover:text-[#4a4a4a]'
                }`}
              >
                <Hash className="w-3.5 h-3.5" /> PIN Sign-In
              </button>
              <button
                type="button"
                onClick={() => {
              ...
              >
                <KeyRound className="w-3.5 h-3.5" /> Password Sign-In
              </button>
            </div>
            ...
            {loginMode === 'pin' ? (
              <form onSubmit={handlePinLogin} className="space-y-4 text-xs">
                <div>
                  ...
                  <div className="relative">
                    <Hash className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-[#6d8196]" />
                    <input
                      type="password"
                      maxLength={6}
                      required
                      ...
                      className="w-full bg-slate-50 border border-[#cbcbcb] rounded-[5px] pl-9 pr-3 py-3 text-lg font-bold font-mono tracking-widest text-[#4a4a4a] focus:outline-none focus:border-[#6d8196] focus:bg-white"
                    />
                  </div>
                </div>
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-emerald-700 hover:bg-emerald-800 text-white font-bold py-3 rounded-[5px] text-xs shadow-sm flex items-center justify-center gap-2 transition-all disabled:opacity-50 mt-2"
                >
                  {loading ? 'Verifying PIN...' : 'Sign In with PIN'} <ArrowRight className="w-4 h-4" />
                </button>
              </form>
            ) : (
              /* Password Login Form */
              <form onSubmit={handlePasswordLogin} className="space-y-4 text-xs">
                ...
                >
                  {loading ? 'Authenticating...' : 'Sign In with Password'} <ArrowRight className="w-4 h-4" />
                </button>
              </form>
            )}
          </div>
